...
export const Route = createFileRoute("/ai-front-desk")({
  head: () => ({
    meta: [
      { title: "AI Front Desk Demo — Creative Launch OS" },
...
function FrontDesk() {
  ...
  useEffect(() => {
    setMessages([
      {
        from: "ai",
        text: "Hi, I'm the AI front desk for Creative Launch OS. To tailor this demo, what kind of creator are you?",
        options: CREATORS,
  ...
  return (
    <PageLayout>
      ...
      <section className="mx-auto max-w-3xl px-6 pb-24">
        <div className="rounded-3xl glass overflow-hidden shadow-elevated">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-border/50">
            <div className="h-9 w-9 grid place-items-center rounded-xl bg-gradient-to-br from-primary to-neon">
              <Bot className="h-4 w-4" />
            </div>
            <div>
              <div className="font-medium text-sm">Creative Launch OS · AI Assistant</div>
              <div className="text-xs text-cyan flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-cyan animate-pulse" /> Online
              </div>
            </div>
            <button
              onClick={handleRestart}
              className="ml-auto inline-flex items-center gap-1.5 rounded-full border border-cyan/50 bg-cyan/10 hover:bg-cyan/20 px-3 py-1.5 text-xs font-medium text-cyan transition-colors"
            >
              ↻ Restart
            </button>
          ...
          <div ref={scrollRef} className="h-[460px] overflow-y-auto px-5 py-6 space-y-3">
            {messages.map((m, i) => (
              <div key={i} className="space-y-2">
                <div className={`flex gap-2 ${m.from === "user" ? "justify-end" : ""}`}>
                  {m.from === "ai" && (
                    <div className="h-7 w-7 grid place-items-center rounded-lg bg-surface border border-border/50 shrink-0">
                  ...
                  <div
                    className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                      m.from === "user"
                        ? "bg-gradient-to-br from-primary to-violet text-primary-foreground"
                        : "bg-surface/80"
                    }`}
                  >
                    {m.text}
                  ...
                  <div className="flex flex-wrap gap-2 pl-9">
                    {m.options.map((opt) => (
                      <button
                        ...
                        className="rounded-full border border-primary/40 bg-primary/10 hover:bg-primary/20 px-3 py-1.5 text-xs font-medium transition-colors"
                      >
                        {opt}
                      </button>
                    ))}
            ...
            {typing && (
              <div className="flex gap-2">
                <div className="h-7 w-7 grid place-items-center rounded-lg bg-surface border border-border/50">
                  <Bot className="h-3.5 w-3.5 text-cyan" />
                </div>
                <div className="rounded-2xl px-4 py-3 bg-surface/80 flex gap-1">
                  {[0, 1, 2].map((i) => (
                    <span
                      key={i}
          ...
          <form onSubmit={handleSend} className="border-t border-border/50 p-4 flex gap-2">
            <input
              ...
              className="flex-1 rounded-full bg-surface/60 border border-border/50 px-4 py-3 text-sm outline-none focus:border-primary/60 transition-colors"
            />
            <button
              type="submit"
              disabled={!input.trim()}
              className="rounded-full bg-gradient-to-r from-primary via-violet to-neon px-5 py-3 text-sm font-semibold disabled:opacity-40 hover:scale-[1.01] transition-transform inline-flex items-center justify-center gap-2"
            >
